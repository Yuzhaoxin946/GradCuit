...
def _require_word2number_for_math_extraction() -> None:
    if w2n is None:
        raise ImportError(
            "word2number is required for Math-500 prompt_idx=0 answer extraction. "
            "Install the evaluation dependencies before running Math-500 experiments."
        )
...
def convert_word_number(text: str) -> str:
    _require_word2number_for_math_extraction()
    try:
...
def extract_theoremqa_answer(pred: str, answer_flag: bool = True):
    if any([option in pred.lower() for option in ["yes", "true"]]):
        ...
        if answer_flag:
            ...
            _require_latex2sympy_for_theoremqa_extraction()
            try:
